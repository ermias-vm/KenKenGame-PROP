...
import static main.presentacio.Utils.*;
...
public class VistaMenuJugarPartida extends JPanel {
    ...
    VistaMenuJugarPartida() {
        ...
        JButton introduirTaulerManualment = crearBotoPersonalitzat(100, 70, "Introduir Tauler a Mà", Color.WHITE, Utils.COLOR_BOTO_BLAU, true);        introduirTaulerManualment.setFont(new Font("Arial", Font.PLAIN, 20));
        ...
        JButton introduirIdentificadorTauler = crearBotoPersonalitzat(100, 70, "Introduir ID Tauler", Color.WHITE, Utils.COLOR_BOTO_BLAU, true);
        introduirIdentificadorTauler.setFont(new Font("Dialog", Font.PLAIN, 24));
        ...
        JButton partidaAleatoria = crearBotoPersonalitzat(100, 70, "Partida Aleatòria", Color.WHITE, Utils.COLOR_BOTO_BLAU, true);
        partidaAleatoria.setFont(new Font("Dialog", Font.PLAIN, 24));
        partidaAleatoria.addActionListener(e -> {
            for (ObservadorBoto ob : observadorsBoto_) ob.jugarPartidaAleatoria();
        });
        JButton ultimaPartidaGuardada = crearBotoPersonalitzat(100, 70, "Última Partida Guardada", Color.WHITE, Utils.COLOR_BOTO_VERD, true);
        ultimaPartidaGuardada.setFont(new Font("Dialog", Font.PLAIN, 24));
        ultimaPartidaGuardada.addActionListener(e -> {
            for (ObservadorBoto ob : observadorsBoto_) ob.jugarUltimaPartidaGuardada();
        });
        JButton partidaGuardada = crearBotoPersonalitzat(100, 70, "Partida Guardada", Color.WHITE, Utils.COLOR_BOTO_VERD, true);
        partidaGuardada.setFont(new Font("Dialog", Font.PLAIN, 24));
        partidaGuardada.addActionListener(e -> {
            for (ObservadorBoto ob : observadorsBoto_) ob.jugarPartidaGuardada();
        });
        JButton tornar = crearBotoMitja("Sortir", Color.WHITE, COLOR_BOTO_VERMELL,true);
